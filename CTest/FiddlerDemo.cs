        ...
        public static void Test2()
        {
            try
            {
                ...
                Fiddler.FiddlerApplication.BeforeRequest += delegate (Fiddler.Session os)
                {
                    ...
                    if (os.oRequest.headers.RequestPath.Contains("/api100/action"))
                    {
                        //os.RequestHeaders.RequestPath = os.RequestHeaders.RequestPath.Replace("xml", "abc");

                        ////捕获并停止
                        //os.utilCreateResponseAndBypassServer();
                        //os.utilSetResponseBody("fiddler");

                        os.bBufferResponse = true;

                        //os.RequestBody = Encoding.UTF8.GetBytes("康");
                        os.utilSetRequestBody("零");
                        os.oRequest.headers["custom"] = "custom data";
                        

                        //remote address
                        IPHostEntry host = Dns.GetHostEntry("ctxdev-ddc.dev.cloud.com");
                ...
                Fiddler.FiddlerApplication.BeforeResponse += (os) =>
                {
                    if (os.oRequest.headers.RequestPath.Contains("/api100/action"))
                    {
                        //捕获并继续
                        ...
                        os.responseCode = 200;
                        //os.utilSetResponseBody("fiddler");
                        os.oResponse.headers["custom"] = "custom data";

                    }
                };

                Fiddler.FiddlerApplication.AfterSessionComplete += (os) =>
                {
                    Console.Title = ("Session list contains: " + oAllSessions.Count.ToString() + " sessions");
                };
