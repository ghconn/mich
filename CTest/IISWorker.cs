...
namespace CTest
{
            ...
            foreach (DirectoryEntry child in ent.Children)
            {
                if (child.SchemaClassName.Equals("IIsWebServer", StringComparison.OrdinalIgnoreCase))
                {
                    if (child.Properties["ServerBindings"].Value != null)
                    {
                        ...
                        iisInfo.ServerBindings = serverBindingStr;
                        var propertyinfo = iisInfo.GetType().GetProperties();
                        foreach(var p in propertyinfo)
                        {
                            p.SetValue(iisInfo, child.Properties[p.Name].Value?.ToString());
                        }
                        iisList.Add(iisInfo);
                    }
                }
    ...
    public class IISInfo
    {
        ...
        public string LogPluginClsid { get; set; }
        public string AppPoolId { get; set; }
        public string TraceUriPrefix { get; set; }
        public string DirBrowseFlags { get; set; }
        public string DefaultDoc { get; set; }
        public string AuthFlags { get; set; }
        public string AnonymousUserName { get; set; }
        public string HttpErrors { get; set; }
        public string AccessSSLFlags { get; set; }
        public string NTAuthenticationProviders { get; set; }
        public string AccessFlags { get; set; }
        public string ScriptMaps { get; set; }
        public string RedirectHeaders { get; set; }
        public string HttpCustomHeaders { get; set; }
        public string ServerState { get; set; }
        public string Win32Error { get; set; }
