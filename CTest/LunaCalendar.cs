...
namespace CTest
{
    class LunaCalendar
    {
        ...
        static void _Main()
        {
            ...
            for (var date = ChineseCalendar.MinSupportedDateTime; date <= DateTime.MaxValue && date <= ChineseCalendar.MaxSupportedDateTime; date = date.AddDays(1))
            {
                if (!dict.ContainsKey(date.Year))
                {
                    dict.Add(date.Year, false);
                }
                ...
                {
                    dict[date.Year] = true;
                    Console.WriteLine($"date {date.ToShortDateString()} 农历：{year} {month} {day}");
                }
                if (date.AddDays(1).Year != date.Year && !dict[date.Year])
                {
            ...
            Console.WriteLine(dict.Count(p => p.Value));
        }
